...
async function main() {
  ...
  for (let i = 0; i < createdLeagues.length; i++) {
    ...
    for (let j = 0; j < teamCount; j++) {
      ...
      // Add players to the team using LeagueService
      await LeagueService.createBotPlayersForTeam(team.id, league.level);

      console.log(`✅ Created team: ${team.name} in ${league.name} with players`);
    }
  }
